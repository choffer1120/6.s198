...
const maximumSamplesAllowed = 99;
...
function onSpecialButtonClick() {
  window.alert('Number of the top ' + 20 +  ' closest matches in each class: ' + globNCounts + '\n Confidence for which the image matches each class: ' + globConf)
}
...
class WebcamClassifier {
  ...
  animate() {
    if (this.isDown) {
      ...
      if (this.current.imagesCount >= maximumSamplesAllowed) {
        window.alert('Maximum samples collected!')
        return;
      }
      ...
      const computeConfidences = () => {
        ...
        let nCounts = [0, 0, 0];
        let rawProbabilities = [0,0,0]
        let confidences = [];
        let scaleSum = 0.0000001;

        for (let index = 0; index < CLASS_COUNT; index += 1) {
          nCounts[index] = classTopKMap[index];
          const probability = classTopKMap[index] / globNCounts[index];
          rawProbabilities[index] = probability;
          scaleSum = scaleSum + probability;
        }

        for (let index = 0; index < CLASS_COUNT; index += 1) {
          confidences[index] = rawProbabilities[index] / scaleSum;
        }
